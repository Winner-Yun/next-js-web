      ...
      {variant === "badge" ? (
        <Badge
          className={cn(
            ...
            value > 0 && "bg-emerald-500/10 text-emerald-500",

            value < 0 && "bg-red-500/10 text-red-500",
...
function DeltaIcon({
  ...
  const iconProps = {
    ...
    ...(filled ? {} : { "data-slot": "delta-icon" as const }),

    ...props,
  };

  const wrap = (icon: React.ReactElement) =>
    filled ? <FilledShell value={value}>{icon}</FilledShell> : icon;

  if (!value) {
    return wrap(<MinusIcon {...iconProps} />);
  }

  if (value > 0) {
    if (variant === "trend") {
      return wrap(<TrendingUpIcon {...iconProps} />);
    }

    if (variant === "arrow") {
      return wrap(<ArrowUpIcon {...iconProps} />);
    }

    return wrap(<ChevronUpIcon {...iconProps} />);
  }

  if (variant === "trend") {
    return wrap(<TrendingDownIcon {...iconProps} />);
  }

  if (variant === "arrow") {
    return wrap(<ArrowDownIcon {...iconProps} />);
  }

  return wrap(<ChevronDownIcon {...iconProps} />);
}
