...
export async function POST(request: Request) {
  try {
    const targetUrl = `${BACKEND_URL.replace(/\/$/, "")}/auth/logout`;
    ...
    if (!contentType.includes("application/json")) {
      ...
      console.error("Backend returned non-JSON response on logout:", errorText);
    ...
    console.error(`Proxy POST logout error:`, error);
